		...
		foreach($this->modules as $key => $module) {
			...
			if($count = count($this->flavors)) {
				...
				foreach($this->flavors as $flavor => $dimension) {
					...
					if(!$dimenPushed) {$dimensionContent .= "'$dimension'";}
					$flavorContent .= $flavor." {\r\n\t\t\tdimension '$dimension'\r\n\t\t}";
					
					if($fdCount < ($count - 1)) {
						$fdCount = $fdCount + 1;
						if(!$dimenPushed) {$dimensionContent .= ", ";}
						$flavorContent .= "\r\n\t\t";
					}
				}
				$flavorContent .= "\r\n\t}\r\n";
			...
			file_put_contents($this->projectName.'\\'.$moduleName.'\build.gradle', 
			...
			file_put_contents($this->projectName.'\\'.$moduleName.'\src\main\AndroidManifest.xml', 
